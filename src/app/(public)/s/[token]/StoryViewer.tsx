...
export default function StoryViewer({ story, cards, routePoints }: StoryViewerProps) {
  ...
  function renderCard(card: StoryCard) {
    switch (card.cardType) {
      ...
      case 'photo':
        return <PhotoCard data={card.data} />;
      case 'text':
        return <TextCard data={card.data} />;
      case 'sub_header':
        return <SubHeaderCard data={card.data} />;
      default: {
        // Graceful fallback for unknown future card types — never a black hole
        const d = card.data as Record<string, unknown>;
        const title = (d.title || d.trip_name || d.text || card.cardType) as string;
        return (
          <div className="flex h-full w-full flex-col items-center justify-center px-8" style={{ backgroundColor: '#F0F2F5' }}>
            <p className="mb-2 text-xs font-semibold uppercase tracking-wider text-gray-400">
              {card.cardType}
            </p>
            <p className="max-w-[300px] text-center text-[18px] font-semibold text-gray-700">
              {title}
            </p>
          </div>
        );
